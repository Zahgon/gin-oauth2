...
Gin-OAuth2 is specially made for [Fiber Framework](https://github.com/gofiber/fiber)
users who also want to use OAuth2. It was created by Go developers
who needed Fiber middleware for working with OAuth2 and couldn't find
any.
...
the Go community. We've liked using Fiber for its speed,
accessibility, and usefulness in developing microservice
architectures. In creating Gin-OAuth2, we wanted to take fuller
advantage of Fiber's capabilities and help other devs do likewise.
...
Assuming you've installed Go and Fiber, run this:
...
Next, define which Fiber middlewares you use. The third line in this
...
Now define which Fiber middlewares you use:
...
team. We'll use a router group again:
	...
	privateGroup.Get("/", func(c *fiber.Ctx) error {
		...
		if team := c.Locals("team"); team != nil && uid != nil {
			return c.JSON(fiber.Map{"message": fmt.Sprintf("Hello from private to %s member of %s", uid, team)})
		}
...
Run example service:
    ...
    I1028 10:12:44.908274   22325 main.go:73] Register allowed users: [{Realm:/employees Uid:sszuecs Cn:Sandor Szücs} {Realm:/employees Uid:njuettner Cn:Nick Jüttner}] and groups: [...] and services: [...]
    I1028 10:12:44.908342   22325 main.go:104] bootstrapped application
     ...
     │ Prefork ....... Disabled  PID ............. 22325 │
     └───────────────────────────────────────────────────┘

    I1028 10:12:46.794502   22325 zalando.go:124] [Gin-OAuth] Grant access to sszuecs
    10:12:46 | 200 |  194.162911ms | 127.0.0.1 | GET | /api/private/ | -
...
As shown in [this great article](http://skarlso.github.io/2016/06/12/google-signin-with-go/) about Google signin in Go, you have to create credentials for an "OAuth client ID." In your [Google Cloud Console](https://console.cloud.google.com), you will find "Credentials" in the "API Manager":
...
Lastly, you have to choose a session name. Fiber's session middleware
keeps the session data server side, so the secret is only kept for API
compatibility and is not used to sign a cookie.
...
With a publicly accessible login resource in place, we can protect a private router group accessibly only with a Google authorized session. Auth will save your user information in the Fiber context local "user."
...
A handler will fetch user information from the fiber.Ctx locals that are stored in google.Auth.

        func UserInfoHandler(ctx *fiber.Ctx) error {
	        return ctx.Status(http.StatusOK).JSON(fiber.Map{"Hello": "from private", "user": ctx.Locals("user")})
        }
...
With a publicly accessible login resource in place, we can protect a private router group accessibly only with a GitHub authorized session. Auth will save your user information in the Fiber context local "user."
...
A handler will fetch user information from the fiber.Ctx locals that are stored in github.Auth.
